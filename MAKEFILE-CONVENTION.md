...
- **Some registries published, others failed:** Do NOT delete tag. Fix the cause, then re-run the failed run in place: `gh run rerun <run-id> --failed` for a run that concluded `failure`, plain `gh run rerun <run-id>` for one that was cancelled or skipped (`--failed` picks failed jobs only, and those runs have none); the orchestrator's error prints the right command per run. Once every re-run has succeeded, re-run `release-github.yml` (`gh run rerun <its-run-id>`); it polls the same push runs again, sees them all succeeded, and creates the GitHub Release. The orchestrator only sees the runs the tag push triggered — a `gh workflow run release-<lang>.yml` dispatch starts a separate dry-run rehearsal that never publishes and is never detected. A re-run builds the tagged commit, so a fix that changes any checked-in file means releasing the next patch instead; so does a language whose workflow was missing or disabled at the tag (no push run exists to re-run).
